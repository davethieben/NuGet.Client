...
{packageReferences}
    ...
    <Import Sdk='NuGet.Build.Tasks.Pack' Project='..\build\NuGet.Build.Tasks.Pack.targets'/>
</Project>";
...
namespace Sample
{
    public static class Program
    {
        ...
        internal void CreateDotnetToolProject(string solutionRoot, string projectName, string targetFramework, string rid, string packageSource = null, IList<PackageIdentity> packages = null, int timeOut = 60000)
        {
            var workingDirectory = Path.Combine(solutionRoot, projectName);
            if (!Directory.Exists(workingDirectory))
            {
